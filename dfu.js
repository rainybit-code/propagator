...
(function () {
  ...
  // DFU states
  const ST = { appIDLE: 0, appDETACH: 1, dfuIDLE: 2, dfuDNLOAD_SYNC: 3, dfuDNBUSY: 4,
  ...
  const STM_VID = 0x0483, STM_DFU_PID = 0xdf11;
  ...
  function parseMemory(desc) {
    ...
    for (let i = 1; i + 1 < parts.length; i += 2) {
      ...
      for (const s of sectors) {
        ...
        const readable = !!(t & 1), erasable = !!(t & 2), writable = !!(t & 4);
        segments.push({ start, end: start + count * size, sectorSize: size, count,
                        readable, erasable, writable });
        start += count * size;
      }
    }
  ...
  class DfuseDevice {
    ...
    _out(request, value, data) {
      return this.dev.controlTransferOut(
        { requestType: 'class', recipient: 'interface', request, value, index: this.ifaceNumber },
        data);
    }
    ...
    async clearStatus() { await this._out(REQ.CLRSTATUS, 0); }
    async abortToIdle() {
      await this._out(REQ.ABORT, 0);
      let s = await this.getStatus();
      if (s.state === ST.dfuERROR) { await this.clearStatus(); s = await this.getStatus(); }
      return s;
    }
    ...
    async erase(startAddr, length, onProgress) {
      ...
      if (seg0) addr = seg0.start + Math.floor((addr - seg0.start) / seg0.sectorSize) * seg0.sectorSize;
      while (addr < end) {
    ...
    async download(data, startAddr, onProgress) {
      ...
      await this.erase(startAddr, total, (r) => onProgress && onProgress({ phase: 'erase', ratio: r }));
    ...
    async connect() {
      ...
      if (this.altSetting != null) {
        try { await this.dev.selectAlternateInterface(this.ifaceNumber, this.altSetting); }
        catch (e) { /* some stacks reject; usually fine */ }
      }
    ...
    async close() {
      try { await this.dev.releaseInterface(this.ifaceNumber); } catch (e) {}
    ...
    async _readTransferSize() {
      ...
      while (i < buf.length) {
        ...
        if (len === 0) break;
        if (type === 0x21 && i + 7 <= buf.length) {       // DFU functional descriptor
    ...
    for (const iface of cfg.interfaces) {
      for (const alt of iface.alternates) {
        if (alt.interfaceClass === 0xfe && alt.interfaceSubclass === 0x01) {
          return { ifaceNumber: iface.interfaceNumber, altSetting: alt.alternateSetting,
  ...
  const WebDFU = {
    STM_VID, STM_DFU_PID,
    supported() { return typeof navigator !== 'undefined' && !!navigator.usb; },
    async requestDevice() {
      if (!this.supported()) throw new Error('WebUSB not available (use Chrome or Edge over https/localhost)');
      const usbDevice = await navigator.usb.requestDevice({
        filters: [{ vendorId: STM_VID, productId: STM_DFU_PID }],
      });
      const info = await pickDfuInterface(usbDevice);
      if (!info) { try { await usbDevice.close(); } catch (e) {} throw new Error('No DFU interface on this device'); }
