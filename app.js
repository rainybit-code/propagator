...
(function buildPatchKnob() {
    ...
    const apply = () => {
        const v = cur();
        dial.style.setProperty('--rot', rotFor(v) + 'deg');
        const b = Math.round((v * 2 - 1) * 100);
        val.textContent = (b > 0 ? '+' : '') + b;
    };
    ...
    const set = (v) => {
        ...
        slotSet(patchSel, null, null, v);
        apply();
        renderPatch();
    };
    dial.addEventListener('pointerdown', (e) => {
        if (patchSel < 0) return;
        drag = true;
        sy = e.clientY;
        sv = cur();
        try {
    ...
    dial.addEventListener('pointermove', (e) => {
        if (drag) set(sv + (sy - e.clientY) / 200);
    });
    const end = (e) => {
        if (!drag) return;
        drag = false;
    ...
    dial.addEventListener(
        ...
        (e) => {
            if (patchSel < 0) return;
            e.preventDefault();
            set(cur() - Math.sign(e.deltaY) * 0.02);
        },
        { passive: false },
    );
})();
...
if (patchRmEl)
    patchRmEl.addEventListener('click', () => {
        if (patchSel < 0) return;
        slotSet(patchSel, 0, 0, 0.5);
        patchSel = -1;
        renderPatch();
    });
function refreshMatrix() {
    renderPatch();
} // called on preset load
buildPatch();
...
function setVar(pos) {
    ...
    activeVar = pos;
    toggleEls[1].dataset.pos = pos;
    updateToggleVals(1, pos);
    sendCC(CONFIG.ccVar, pos / 2);
}
function setMasterFilt(type) {
    // master filter type 0 off / 1 LP / 2 BP / 3 HP -> CC 88
    activeMasterFilt = type;
    document
        .querySelectorAll('#masterFiltSeg button')
...
[0, 1].forEach((si) => {
    ...
    const led = el('span', 'fs-led');
    led.dataset.led = si;
    const s = el('div', 'stomp');
    s.dataset.stomp = si;
    const lbl = el('div', 'stomp-name');
    stompNames[si] = lbl;
    stompEls[si] = s;
    ledEls[si] = led;
    s.addEventListener('click', () => {
        if (si === 0) {
            // FS1 = engage/bypass (latching)
            const on = !s.classList.contains('pressed');
            s.classList.toggle('pressed', on);
            led.classList.toggle('on', on);
            sendCC(CONFIG.ccFs1, on ? 1 : 0); // >=64 -> bypassed
        } else {
            // FS2 = momentary mode action
...
function setMode(m) {
    activeMode = m;
    toggleEls[0].dataset.pos = m;
    updateToggleVals(0, m);
    ...
    modeKnobs.forEach((k, i) => {
        k._label.textContent = labels[i];
    });
...
function setFx(f) {
    activeFx = f;
    toggleEls[2].dataset.pos = f;
    updateToggleVals(2, f);
    document
        .querySelectorAll('#fxSeg button')
        .forEach((b) => b.classList.toggle('on', +b.dataset.fx === f));
    const fp = $('#fxPod');
    ...
    if (typeof podShown !== 'undefined' && !podShown.fxPod) {
        podShown.fxPod = true;
        const fm = podMeta('fxPod');
        if (fm && fm._cb) fm._cb.checked = true;
        saveView();
        reopened = true;
    }
...
function capturePatch() {
    return {
        ...
        mode: activeMode,
        fx: activeFx,
        delaySync: delaySyncIdx,
        var: activeVar,
        masterFilt: activeMasterFilt,
        knobs: {
            mode: knobValue.mode.slice(),
            fx: knobValue.fx.slice(),
            synth: knobValue.synth.slice(),
...
function applyPatch(p) {
    if (!p || !p.knobs) return;
    if (Array.isArray(p.knobs.mode))
        p.knobs.mode.forEach((v, i) => {
            if (i < knobValue.mode.length) knobValue.mode[i] = v;
        });
    if (Array.isArray(p.knobs.fx))
        p.knobs.fx.forEach((v, i) => {
            if (i < knobValue.fx.length) knobValue.fx[i] = v;
        });
    // tolerate older/shorter synth arrays: keep defaults for any missing tail params
    if (Array.isArray(p.knobs.synth))
        p.knobs.synth.forEach((v, i) => {
            if (i < knobValue.synth.length) knobValue.synth[i] = v;
        });
    if (Array.isArray(p.knobs.chaos))
        p.knobs.chaos.forEach((v, i) => {
            if (i < knobValue.chaos.length) knobValue.chaos[i] = v;
        });
    if (Array.isArray(p.knobs.master))
        p.knobs.master.forEach((v, i) => {
            if (i < knobValue.master.length) knobValue.master[i] = v;
        });
    if (Array.isArray(p.knobs.gen))
        p.knobs.gen.forEach((v, i) => {
            if (i < knobValue.gen.length) knobValue.gen[i] = v;
        });
    if (Array.isArray(p.knobs.gran))
        p.knobs.gran.forEach((v, i) => {
            if (i < knobValue.gran.length) knobValue.gran[i] = v;
        });
    ...
    const pv = p.v || 1;
    if (pv < 2)
        PATCH_SLOTS.forEach((b) => {
            knobValue.synth[b] = Math.round(knobValue.synth[b] * 6) / 7;
        });
    if (pv < 3)
        PATCH_SLOTS.forEach((b) => {
            knobValue.synth[b] = Math.round(knobValue.synth[b] * 7) / 8;
        });
...
function loadPresetByKey(key) {
    if (!key) return;
    if (key[0] === 'f') {
        const p = factoryPresets.find((x) => x.name === key.slice(2));
        if (p) applyPatch(p.patch);
    } else {
        const u = loadUserPresets();
        if (u[key.slice(2)]) applyPatch(u[key.slice(2)]);
    }
}
...
function closeSaver() {
    presetSaver.hidden = true;
    presetSaveBtn.hidden = false;
    document.removeEventListener('pointerdown', saverOutside, true);
}
function saverOutside(e) {
    if (!presetSaver.contains(e.target)) closeSaver();
}
function openSaver() {
    presetSaveBtn.hidden = true;
    presetSaver.hidden = false;
...
$('#presetDel').addEventListener('click', () => {
    const sel = $('#presetSel'),
        key = sel.value;
    if (key[0] !== 'u') {
        alert('Pick one of your saved presets to delete (factory presets are read-only).');
        return;
    }
...
function scheduleAutosave() {
    ...
    autosaveTimer = setTimeout(() => {
        try {
            localStorage.setItem(AUTOSAVE_KEY, JSON.stringify(capturePatch()));
        } catch (_) {}
    }, 300);
}
...
function importPresets(file) {
    ...
    r.onload = () => {
        try {
            ...
            const u = loadUserPresets();
            let n = 0;
            for (const [name, patch] of Object.entries(obj))
                if (patch && patch.knobs) {
                    u[name] = patch;
                    n++;
                }
...
async function loadFactoryPresets() {
    try {
        ...
        /* offline / file:// — user presets still work */
    }
    rebuildPresetList();
}
...
function sendCC(cc, v01) {
    ...
    try {
        midiOut.send([0xb0 | (CONFIG.channel & 0x0f), cc, v]);
    } catch (_) {}
}
...
function bindInputs() {
    ...
    midi.inputs.forEach((inp) => {
        inp.onmidimessage = inp === midiIn || inp === sporeIn ? onMidiMessage : null;
    });
}
...
function refreshDevices() {
    ...
    const ko = el('option');
    ko.value = 'kbd';
    ko.textContent = '⌨ Computer keyboard';
    ...
    let savedOut = '',
        ...
        savedInName = '';
    try {
        savedOut = localStorage.getItem('propagator.out') || '';
        savedOutName = localStorage.getItem('propagator.outName') || '';
        savedIn = localStorage.getItem('propagator.in') || '';
        savedInName = localStorage.getItem('propagator.inName') || '';
    } catch (_) {}
    if (!midiOut) {
        ...
        if (outId) {
            selectOut(outId);
            $('#midiOut').value = outId;
        } else if (midi.outputs.size) {
            const first = midi.outputs.values().next().value;
            selectOut(first.id);
            $('#midiOut').value = first.id;
        }
    }
    if (!midiIn && !kbdOn) {
        if (savedIn === 'kbd') {
...
function updateConn() {
    ...
    $('#footMidi').textContent = midi
        ? `${midi.outputs.size} out · ${midi.inputs.size} in`
        : 'no MIDI';
    const dfu = $('#dfuBtn');
    if (dfu) dfu.disabled = false; // flash wizard uses WebUSB, not MIDI
    // THRU only makes sense with a distinct IN + OUT — disable the toggle otherwise
    const thruEl = $('#midiThru');
    const thruField = thruEl && thruEl.closest('.switch-field');
...
function selectOut(id) {
    ...
    updateConn();
    if (midiOut) {
        // sync Spore to the UI's current state on connect
...
function kbdSend(note, vel, on) {
    if (!midiOut) return;
    try {
        midiOut.send([(on ? 0x90 : 0x80) | (CONFIG.channel & 0x0f), note, vel]);
    } catch (_) {}
}
function kbdDown(e) {
    ...
    if (k === 'z') {
        kbdOctave = Math.max(-3, kbdOctave - 1);
        return;
    }
    if (k === 'x') {
        kbdOctave = Math.min(3, kbdOctave + 1);
        return;
    }
    if (!(k in KBD_MAP) || kbdHeld.has(k)) return;
    const note = 60 + KBD_MAP[k] + kbdOctave * 12;
    kbdHeld.set(k, note);
    kbdSend(note, 100, true);
    if (midiAct) {
        midiAct.classList.add('kick');
        setTimeout(() => midiAct.classList.remove('kick'), 130);
    }
    e.preventDefault();
}
function kbdUp(e) {
    ...
    kbdSend(kbdHeld.get(k), 0, false);
    kbdHeld.delete(k);
}
function kbdPanic() {
    kbdHeld.forEach((note) => kbdSend(note, 0, false));
    kbdHeld.clear();
}
function setKbd(on) {
    if (on === kbdOn) return;
...
function resolveSaved(map, savedId, savedName) {
    ...
    if (savedName) {
        for (const p of map.values()) if ((p.name || '') === savedName) return p.id;
    }
    return '';
}
...
function onMidiMessage(e) {
    ...
    if (status === 0xf0 && d[1] === 0x7d) {
        if (d.length >= 4 && d[2] === 0x41) {
            // identify reply: F0 7D 41 <version ASCII> F7
            let s = '';
            ...
            checkFwUpdate();
        } else if (d.length >= 5 && d[2] === 0x42) {
            // CPU load: F0 7D 42 <avg%> <max%> F7
            showCpuLoad(d[3], d[4]);
    ...
    if ((status & 0xf0) === 0xb0) {
        const cc = d[1],
            val = d[2];
        const mi = CONFIG.ccMode.indexOf(cc),
            fi = CONFIG.ccFx.indexOf(cc);
        const third = (v) => (v < 43 ? 0 : v < 86 ? 1 : 2);
        applyingRemote = true;
        try {
            if (mi >= 0) {
                knobValue.mode[mi] = val / 127;
                modeKnobs[mi]._apply();
            } else if (fi >= 0) {
                knobValue.fx[fi] = val / 127;
                ...
                const on = val >= 64;
                stompEls[0].classList.toggle('pressed', on);
                ledEls[0].classList.toggle('on', on);
            }
        } finally {
...
async function initMidi() {
    if (!navigator.requestMIDIAccess) {
        ...
        if (location.protocol === 'file:') {
            body =
                'You opened this as a <code>file://</code> page — browsers hide Web MIDI there. ' +
            ...
            body =
                `This page isn't a secure context (<code>${location.protocol}//${location.host}</code>). ` +
            ...
            body =
                'This browser has no Web MIDI API. Use <strong>Chrome</strong> or <strong>Edge</strong> ' +
                '(Safari and older Firefox lack it).';
        }
        console.warn(
            '[propagator] requestMIDIAccess missing · protocol=%s host=%s secureContext=%s',
            location.protocol,
            location.host,
    ...
    try {
        ...
        try {
            ...
            showNotice(
                ...
                'Allow MIDI access (and serve over <code>localhost</code> or https for SysEx), then retry.',
            );
            return;
        }
    }
...
$('#midiThru').addEventListener('change', (e) => {
    thru = e.target.checked;
    updateMidiPod();
});
$('#fNotes').addEventListener('change', (e) => {
    thruFilter.notes = e.target.checked;
});
$('#fCC').addEventListener('change', (e) => {
    thruFilter.cc = e.target.checked;
});
$('#fOther').addEventListener('change', (e) => {
    thruFilter.other = e.target.checked;
});
$('#fClock').addEventListener('change', (e) => {
    thruFilter.clock = e.target.checked;
});
$('#fSync').addEventListener('change', (e) => {
    clockSync = e.target.checked;
    if (!clockSync) {
        lastClockMs = 0;
        clockBpm = 0;
    }
...
const bpmNum = $('#bpmNum'),
    beatSeed = $('#beatSeed'),
    beatDots = [...document.querySelectorAll('#beatDots i')];
const bpmMini = $('#bpmMini'),
    bpmMiniLed = $('#bpmMiniLed'),
    bpmMiniVal = $('#bpmMiniVal');
...
function setBpm(v) {
    ...
    if (clockMaster === 'gui' && midiOut)
        sendCC(CONFIG.ccTempo, Math.max(0, Math.min(1, (bpm - 40) / 160)));
}

function updateTransportUI() {
    const b1 = $('#bpmMiniPlay');
    if (b1) b1.textContent = playing ? '⏹' : '▶';
    const b2 = $('#bpmPlay');
    if (b2) b2.textContent = playing ? '⏹ stop' : '▶ start';
}
function setPlaying(p) {
    ...
    if (clockMaster === 'gui' && midiOut) {
        try {
            midiOut.send([playing ? 0xfa : 0xfc]);
        } catch (_) {}
    }
    if (playing) {
        // START: restart from the beginning of the bar (downbeat)
        beatIndex = 0;
        nextBeat = performance.now();
    } else {
        // STOP: clear the beat visuals
...
(() => {
    ...
    dial.addEventListener('pointermove', (e) => {
        if (drag) setBpm(sb + (sy - e.clientY) * 0.5);
    });
...
function startClockEmit() {
    ...
    clockTimer = setInterval(() => {
        ...
        const now = performance.now(),
            tickMs = 60000 / (bpm * 24);
        let guard = 0;
        while (now - clockTickAt >= tickMs && guard++ < 96) {
            try {
                midiOut.send([0xf8]);
            } catch (_) {}
            clockTickAt += tickMs;
...
function setClockMaster(c) {
    clockMaster = c;
    document
        .querySelectorAll('#clockMasterSeg button')
        .forEach((b) => b.classList.toggle('on', b.dataset.c === c));
    ...
    try {
        localStorage.setItem('propagator.clock', c);
    } catch (_) {}
}
$('#clockMasterSeg').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (b) setClockMaster(b.dataset.c);
});
function setDelaySync(s) {
    delaySyncIdx = s;
    document
        .querySelectorAll('#delaySyncSeg button')
        .forEach((x) => x.classList.toggle('on', +x.dataset.s === s));
    sendCC(CONFIG.ccDelaySync, s / 4);
}
$('#delaySyncSeg').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (b) setDelaySync(+b.dataset.s);
});
try {
    setClockMaster(localStorage.getItem('propagator.clock') || 'off');
} catch (_) {
    setClockMaster('off');
}
...
let seqIdx = 0,
    seqDir = 1,
    seqClockTick = 0,
    seqNext = performance.now(),
    curStep = -1;

function seqTotal() {
    return seq.stepsPerBar * seq.bars;
} // total steps in the loop
function seqStepMs(bpmV) {
    return (4 * (60000 / bpmV)) / seq.stepsPerBar;
} // a bar = 4 beats
function seqTicksPerStep() {
    return Math.max(1, Math.round(96 / seq.stepsPerBar));
} // 96 PPQN per bar
function degToMidi(deg) {
    const sc = SCALES[seq.scale].steps,
        L = sc.length;
    const oct = Math.floor(deg / L),
        idx = ((deg % L) + L) % L;
    return 12 * (seq.oct + 1) + seq.root + sc[idx] + 12 * oct;
}
const cellKey = (step, deg) => step + ':' + deg;
function hasNote(step, deg) {
    return seq.cells.has(cellKey(step, deg));
}
function setNote(step, deg, on) {
    const k = cellKey(step, deg);
    if (on) seq.cells.add(k);
    else seq.cells.delete(k);
}
function notesAt(step) {
    const out = [];
    for (let d = 0; d < ROWS; d++) if (hasNote(step, d)) out.push(d);
    return out;
}
...
function stepOfCell(c) {
    return seq.viewedBar * seq.stepsPerBar + +c.dataset.col;
}
function buildGrid() {
    ...
    for (let row = 0; row < ROWS; row++) {
        ...
        const lab = el('span', 'seq-rlabel');
        lab.dataset.deg = deg;
        seqGridEl.appendChild(lab);
        for (let col = 0; col < seq.stepsPerBar; col++) {
            const c = el('span', 'seq-cell');
            c.dataset.col = col;
            c.dataset.deg = deg;
...
function paintGrid() {
    seqGridEl.querySelectorAll('.seq-rlabel').forEach((l) => {
        l.textContent = noteName(degToMidi(+l.dataset.deg));
    });
    seqGridEl.querySelectorAll('.seq-cell').forEach((c) => {
        const step = stepOfCell(c),
            deg = +c.dataset.deg;
...
function paintCursor() {
    seqGridEl.querySelectorAll('.seq-cell').forEach((c) => {
        c.classList.toggle('cur', seq.on && stepOfCell(c) === curStep);
    });
    if (seqBarTabsEl)
        seqBarTabsEl
            ...
            .forEach((b) =>
                b.classList.toggle(
                    ...
                    seq.on &&
                        curStep >= 0 &&
                        +b.dataset.bar === Math.floor(curStep / seq.stepsPerBar),
                ),
            );
}

/* click / drag to paint notes */
let seqPainting = false,
    seqPaintVal = true;
seqGridEl.addEventListener('pointerdown', (e) => {
    ...
    setNote(step, deg, seqPaintVal);
    c.classList.toggle('on', seqPaintVal);
    seqPainting = true;
});
...
function wireSeqSeg(sel, attr, cb) {
    ...
    root.addEventListener('click', (e) => {
        ...
        root.querySelectorAll('button').forEach((x) =>
            x.classList.toggle('on', +x.dataset[attr] === v),
        );
        cb(v);
        saveSeq();
    });
}
wireSeqSeg('#seqScaleSeg', 's', (v) => {
    seq.scale = v;
    paintGrid();
});
wireSeqSeg('#seqOrderSeg', 'o', (v) => {
    seq.order = v;
});
wireSeqSeg('#seqGateSeg', 'g', (v) => {
    seq.gate = v;
});
wireSeqSeg('#seqStepsSeg', 'p', (v) => {
    seq.stepsPerBar = v;
    if (seq.viewedBar >= seq.bars) seq.viewedBar = 0;
    buildGrid();
});
wireSeqSeg('#seqBarsSeg', 'b', (v) => {
    seq.bars = v;
    if (seq.viewedBar >= seq.bars) seq.viewedBar = seq.bars - 1;
    buildBarTabs();
    paintGrid();
});
$('#seqRoot').addEventListener('change', (e) => {
    seq.root = +e.target.value;
    paintGrid();
    saveSeq();
});
$('#seqOct').addEventListener('change', (e) => {
    seq.oct = +e.target.value;
    paintGrid();
    saveSeq();
});
...
$('#seqSeed').addEventListener('click', () => {
    ...
    for (let s = 0; s < total; s++)
        if (Math.random() < 0.4) setNote(s, Math.floor(Math.random() * ROWS), true);
    paintGrid();
    saveSeq();
});
...
function setSeqOn(on) {
    seq.on = on;
    if (seqRunBtn) {
        seqRunBtn.classList.toggle('on', on);
        seqRunBtn.textContent = on ? '⏹ stop' : '▶ run';
    }
    if (on) {
        seqIdx = 0;
        seqDir = 1;
...
function seqNextIdx(i) {
    ...
    switch (seq.order) {
        ...
        case 3:
            return Math.floor(Math.random() * N); // random
        default:
            return (i + 1) % N; // forward
    }
}
function seqFire(stepMs) {
    ...
    if (degs.length && midiAct) {
        midiAct.classList.add('kick');
        setTimeout(() => midiAct.classList.remove('kick'), 90);
    }
    const offMs = Math.max(20, stepMs * SEQ_GATES[seq.gate]);
    degs.forEach((d) => {
        const note = degToMidi(d);
        kbdSend(note, 100, true);
        setTimeout(() => kbdSend(note, 0, false), offMs);
    });
    paintCursor();
...
function saveSeq() {
    try {
        localStorage.setItem(SEQ_KEY, JSON.stringify(serializeSeq()));
    } catch (_) {}
    scheduleAutosave();
}
function loadSeqState(o) {
    if (!o) return;
    ['scale', 'root', 'oct', 'order', 'gate', 'stepsPerBar', 'bars'].forEach((k) => {
        if (typeof o[k] === 'number') seq[k] = o[k];
    });
...
function refreshSeqUI() {
    ...
    const rs = $('#seqRoot');
    if (rs) rs.value = seq.root;
    const os = $('#seqOct');
    if (os) os.value = seq.oct;
    buildBarTabs();
    buildGrid();
}
const SEQ_DEFAULT = serializeSeq(); // pristine sequence (default riff) for "new"
try {
    loadSeqState(JSON.parse(localStorage.getItem(SEQ_KEY) || 'null'));
} catch (_) {}
refreshSeqUI();
...
function startBoil() {
    ...
    setInterval(() => {
        seed = (seed % 6) + 1;
        turbs.forEach((t) => t.setAttribute('seed', seed));
    }, 115);
}
...
try {
    Object.assign(podShown, JSON.parse(localStorage.getItem(VIEW_KEY) || '{}'));
} catch (_) {}
function saveView() {
    try {
        localStorage.setItem(VIEW_KEY, JSON.stringify(podShown));
    } catch (_) {}
}
function podMeta(id) {
...
function applyPods() {
    PODS.forEach((p) => {
        ...
        if (p.tempo) {
            setBpmPodClosed(!want);
        } else {
            pe.hidden = false;
            pe.style.display = want ? '' : 'none';
        }
    });
    layoutPods();
...
function podLaidOut(pe) {
    return pe && getComputedStyle(pe).display !== 'none' && !pe.classList.contains('closed');
}
...
function layoutPods() {
    ...
    ['L', 'R'].forEach((side) => {
        ...
        PODS.filter((p) => p.col === side).forEach((p) => {
            const pe = $('#' + p.id);
            if (!pe || !podLaidOut(pe)) return;
            if (pe.dataset.pinned === '1') {
                clampPod(pe);
                return;
            } // dragged: leave it put
...
function makeDraggable(pod) {
    ...
    pod.addEventListener('pointerdown', (e) => {
        if (
            e.target.closest(
                '.knob, button, input, label, .pill, .switch-field, .bpm-dial, select, .seg, .seq-grid, .env-graph, .patchbay, .beat-seed',
            )
        )
            return;
        dragging = true;
    ...
    pod.addEventListener('pointermove', (e) => {
        ...
        pod.dataset.x = ox + (e.clientX - sx);
        pod.dataset.y = oy + (e.clientY - sy);
        applyPod(pod);
    });
    const end = (e) => {
        if (!dragging) return;
        ...
        try {
            pod.releasePointerCapture(e.pointerId);
        } catch (_) {}
    };
...
PODS.forEach((p) => {
    const pe = $('#' + p.id);
    if (!pe) return;
    const btn = el('button', 'pod-close');
    btn.textContent = '×';
    btn.title = 'hide (re-open from View)';
    btn.addEventListener('click', (e) => {
        e.stopPropagation();
        podShown[p.id] = false;
        if (p._cb) p._cb.checked = false;
        saveView();
        applyPods();
    });
    pe.appendChild(btn);
...
function resetPods() {
    PODS.forEach((p) => {
        const pe = $('#' + p.id);
        if (pe) {
            delete pe.dataset.pinned;
            delete pe.dataset.x;
            delete pe.dataset.y;
        }
    });
    podShown = Object.assign({}, VIEW_DEFAULT);
    saveView();
    PODS.forEach((p) => {
        if (p._cb) p._cb.checked = !!podShown[p.id];
    });
    applyPods();
}
...
function buildViewMenu() {
    ...
    PODS.forEach((p) => {
        ...
        const cb = el('input');
        cb.type = 'checkbox';
        cb.checked = !!podShown[p.id];
        cb.addEventListener('change', () => {
            podShown[p.id] = cb.checked;
            saveView();
            applyPods();
        });
        const sp = el('span');
...
document.addEventListener('click', (e) => {
    const m = $('#viewMenu');
    if (m && !m.hidden && !e.target.closest('.view-wrap')) m.hidden = true;
});
buildViewMenu();
...
function fwStatus(msg, kind) {
    const s = fwEl('fwStatus');
    if (!s) return;
    s.textContent = msg;
    s.dataset.kind = kind || '';
}
function fwSetFlashEnabled() {
    const b = fwEl('fwFlash');
    if (b) b.disabled = !(FLASH.buf && FLASH.dev && !FLASH.mismatch);
    const l = fwEl('fwLeave');
    if (l) l.disabled = !FLASH.dev; // can reboot whenever connected
}
function fwProgress(p) {
    const wrap = fwEl('fwProgWrap'),
        bar = fwEl('fwBar');
    ...
    fwStatus(
        p.phase === 'erase'
            ? 'Erasing…'
            : p.phase === 'write'
              ? 'Writing ' + Math.round((p.ratio || 0) * 100) + '%'
              : 'Finishing…',
    );
}
...
function checkFwUpdate() {
    ...
    const avail = !!(
        ...
        latestFw.version &&
        connectedFw &&
        semverGt(latestFw.version, connectedFw)
    );
        ...
        if (r.ok) {
            ...
            if (j && j.version)
                latestFw = {
                    ...
                    size: j.size,
                    file: j.file || 'spore-latest.bin',
                    bootFile: j.bootFile || null,
                    bootSize: j.bootSize || null,
                };
        }
    } catch (e) {
        /* not deployed / local dev — fine */
    }
    checkFwUpdate();
}
...
function sendIdentify() {
    if (!midiOut) return;
    try {
        midiOut.send([0xf0, 0x7d, 0x01, 0xf7]);
    } catch (e) {
        /* sysex not permitted */
    }
}
...
function sendCpuQuery() {
    if (!midiOut) return;
    try {
        midiOut.send([0xf0, 0x7d, 0x02, 0xf7]);
    } catch (e) {
        /* sysex not permitted */
    }
}
...
function showCpuLoad(avg, max) {
    const meter = $('#cpuMeter'),
        fill = $('#cpuFill'),
        peak = $('#cpuPeak'),
        val = $('#cpuVal');
    if (!meter) return;
    const a = Math.max(0, Math.min(100, avg)),
        m = Math.max(0, Math.min(100, max));
...
function stopCpuPoll() {
    ...
    const meter = $('#cpuMeter');
    if (meter) meter.classList.remove('on', 'cap');
}
...
function pushChaosSample(x7, z7) {
    // bytes 0..127 -> -1..1
...
function drawChaos() {
    ...
    if (n) {
        ...
        ctx.beginPath();
        ctx.arc(px(h.x), py(h.z), 2.2, 0, Math.PI * 2);
        ctx.fill();
    }
}
...
function fwOpen() {
    ...
    // reset transient UI
    fwEl('fwProgWrap').hidden = true;
    fwEl('fwBar').style.width = '0%';
    fwStatus(
        WebDFU.supported() ? '—' : 'WebUSB unavailable — use Chrome/Edge over https/localhost',
        'err',
    );
...
fwEl('fwModeToggle').addEventListener('click', () => {
    ...
    FLASH.buf = null;
    FLASH.bufName = '';
    FLASH.mismatch = false; // re-pick the right .bin
    const fi = fwEl('fwFileInfo');
...
fwEl('fwUseLatest').addEventListener('click', async () => {
    ...
    try {
        ...
        FLASH.buf = await r.arrayBuffer();
        FLASH.bufName = file;
        info.innerHTML =
            '✓ loaded ' +
            (isApp ? '<b>app v' + latestFw.version + '</b>' : '<b>bootloader</b>') +
            ' · ' +
            (FLASH.buf.byteLength / 1024).toFixed(1) +
            ' KB';
        fwSetFlashEnabled();
    } catch (e) {
...
fwEl('fwFile').addEventListener('change', async (e) => {
    const f = e.target.files && e.target.files[0];
    if (!f) return;
    FLASH.buf = await f.arrayBuffer();
    FLASH.bufName = f.name;
    fwEl('fwFileInfo').textContent =
        '✓ ' + f.name + ' · ' + (FLASH.buf.byteLength / 1024).toFixed(1) + ' KB';
    fwSetFlashEnabled();
});
...
fwEl('fwReboot').addEventListener('click', () => {
    if (!midiOut) {
        fwStatus('connect Spore as MIDI OUT first (or use BOOT+RESET)', 'err');
        return;
    }
    const isApp = FLASH.mode === 'app';
...
fwEl('fwConnect').addEventListener('click', async () => {
    try {
        ...
        if (has(FW_ADDR.app)) {
            target = 'app';
            FLASH.addr = FW_ADDR.app;
        } else if (has(FW_ADDR.boot)) {
            target = 'boot';
            FLASH.addr = FW_ADDR.boot;
        } else {
            FLASH.addr = FW_ADDR[FLASH.mode];
        } // unknown map — trust the selected mode
        ...
        if (target && target !== FLASH.mode) {
            ...
            fwStatus(
                'connected to the ' +
                    (target === 'app' ? 'Daisy bootloader (app slot)' : 'STM ROM (internal)') +
                    ', but you picked “' +
                    (FLASH.mode === 'app' ? 'app update' : 'bootloader install') +
                    '”. Switch the target to match, then flash.',
                'err',
            );
        } else {
            fwStatus(
                'connected' + (name ? ' · ' + name : '') + ' · → ' + at + ' · ready to flash',
                'ok',
            );
        }
        fwSetFlashEnabled();
        ...
        fwStatus(
            'connect failed: ' +
                e.message +
                (/no device|chosen/i.test(e.message) ? '' : ' (Windows: WinUSB via Zadig?)'),
            'err',
        );
    }
});
