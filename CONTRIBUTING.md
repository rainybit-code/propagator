...
4. Open a PR against `main`. On every PR, CI syntax-checks `app.js`, validates `presets.json`,
   checks Prettier formatting, and requires a `CHANGELOG.md` entry (label the PR
   `skip-changelog` for changes that don't warrant one).
